...
// 멤버변수
	...
	public ChatClientWindow(String name, Socket socket) {
		...
		this.socket = socket;
		/*
		 * 스레드 작동 1. 내부 클래스, 'UI에서 많이 쓰이는구나'라고 이해) 2. 서버에서 보내온 데이터를 읽어서 TextArea에 출력해주는
		 * 스레드
		 */
	...
	public void show() {
		...
		/*
		 * 람다식으로 하면, buttonSend.addActionListener( (ActionEvent actionEvent) -> {
		 * sendMessage(); } );
		 */
		// Textfield(채팅 입력하는 창)
	...
	private void sendMessage() {
		...
		try {
			/*
			 * 아래 내용을 한번에 한 것 OutputStream outputStream = socket.getOutputStream();
			 * OutputStreamWriter outputStreamWriter = new OutputStreamWriter(outputStream,
			 * StandardCharsets.UTF_8); PrintWriter printWriter = new
			 * PrintWriter(outputStreamWriter, true); // ,true를 주면 '자동 flush'됨
			 */
