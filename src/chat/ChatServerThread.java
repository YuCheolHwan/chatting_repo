...
public class ChatServerThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			// socket을 통해서 클라이언트부터 오는 데이터를 받기위한 스트림 설정
			inputStream = this.socket.getInputStream();
			/*
			 * BufferedReader는 속도 개선 스트림이므로 InputStreamReader에서 문자셋 정해줘야 함 UTF-8은 1byte 또는
			 * 3byte로 읽고, UTF-16은 모두 2byte로 읽음(낭비 발생하므로 UTF-8 사용)
			 */
			inputStreamReader = new InputStreamReader(inputStream, StandardCharsets.UTF_8);
			bufferedReader = new BufferedReader(inputStreamReader);
			// 소켓을 통해서 클라이언트에게 메세지를 전달 할 스트림 설정
			...
			while (true) {
				 ...
				 * 클라이언트로부터 데이터 입력 받을 때까지 무한 대기. 여기서 소켓을 닫으면
				 * 예외처리로 넘어감
				 */
				String messageReceive = bufferedReader.readLine();

				/*
				 * 2-1. 클라이언트가 갑자기 연락(클라이언트 소켓)이 끊어지면 readLine()은 'null'을 읽어온다. 클라이언트가 글을 쓴 후
				 * Enter 칠 때까지 무한 대기
				 */
				...
				/*
				 * 2-2-1. join(클라이언트가 메세지 방에 참가하기) - 패턴화 되어있음 tokens[0]: join, tokens[1]:
				 * nickname
				 */
